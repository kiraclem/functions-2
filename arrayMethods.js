...
const totalPopulation = populations.reduce((acc, num) => acc + num) 
...
const myStrongest = monstersInYourPocket.filter((chicken) => {
  let {CP} = chicken
  return CP >= 200
})
...
// test if bob is the owner in each object, if bob is the owner add the price up 

const BobPrice = purchases.filter(p => p.owner === 'Bob').reduce((bp, l) => (bp + l.price), 0)
